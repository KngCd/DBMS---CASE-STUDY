...
<div class="main-body">
          ...
          foreach ($results as $row) {
              ...
              elseif ($row['type'] === 'activity') {
                if ($row['filetype'] === 'pdf') {
                    ...
                    echo '<form action="view-acts.php" method="post" class="form" id="class-stream-form-view-' . $row['classcode'] . '">';
                    echo '   <input type="hidden" name="classcode" value="' . $row['classcode'] . '">';
                    echo '   <input type="hidden" name="act_id" value="' . $row['act_id'] . '">';
                    echo '   <a href="#" class="submit-button view-act" data-form-id="class-stream-form-view-'. $row['classcode']. '" data-act-id="' . $row['act_id'] . '">';
                    echo '       <i class="fa-solid fa-file-import"></i>';
                    echo '   </a>';
                    echo '</form>';
                    
                    echo '<form action="repost.php" method="post" class="form" id="class-stream-form-repost-' . $row['classcode'] . '">';
                    echo '   <input type="hidden" name="classcode" value="' . $row['classcode'] . '">';
                    echo '   <input type="hidden" name="act_id" value="' . $row['act_id'] . '">';
                    echo '   <a href="#" class="submit-button repost" data-form-id="class-stream-form-repost-'. $row['classcode']. '" data-act-id="' . $row['act_id'] . '" data-class="' . $row['classcode'] . '">';
  ...
  $(document).ready(function() {
    $('.submit-button.view-act').click(function(e) {
      e.preventDefault();
 ...
 $('.submit-button.repost').click(function(e) {
    e.preventDefault();

    var formId = $(this).data('form-id');
    var actId = $(this).data('act-id');
    var classcode = $(this).data('class'); // Modify this line

    // Set the act_id and classcode values in the repost form
    $('#' + formId + ' [name="act_id"]').val(actId);
    $('#' + formId + ' [name="classcode"]').val(classcode);

    $('#' + formId).submit();

    // Redirect to the next page
    setTimeout(function() {
      window.location.href = "repost.php";
    }, 100);
  });
  });
</script>
